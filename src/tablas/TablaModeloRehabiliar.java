...
public class TablaModeloRehabiliar implements TableModel{

	private final static String CLASE = TablaModeloRehabiliar.class.getName();
	protected static Logger log = Logger.getLogger(CLASE);
    private final String[] COLUMNAS = {"ID", "Tarea", "Habilitar"};
    ...
    @SuppressWarnings("rawtypes")
	public TablaModeloRehabiliar(GestorTareas gt) throws Exception{
    	...
    	try {
	        ...
	        switch (columnIndex)
	        {
	            ...
	            case 1:
	                ...
	                break;
	            case 2:
	                tarea.setBajaLogica((Boolean)aValue);
	                break;
//	            case 3:
...
//	                break;
	...
	private void anyadirDatosTabla() throws Exception {
        try{
        	...
        	for (Entry<Integer, TareaBean> obj : objGT.obtenerTareasListado().entrySet()) {
        		if(obj.getValue().isBajaLogica()){
 
	    	        datos.add (obj.getValue());
     ...
     */
	@SuppressWarnings("unchecked")
	public void anyadeTarea(TareaBean tarea) {
		try{ 
		    datos.add (tarea);
